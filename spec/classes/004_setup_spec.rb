...
describe 'spacewalk::setup' do
  ...
  context 'with params' do
    ...
    describe 'with defaults' do
      ...
      it do
        is_expected.to contain_file('/usr/bin/spacewalk.sh').with(content: %r{^spacewalk-setup.+--answer-file=\/etc\/sysconfig\/spacewalk\.answer\s+?--non-interactive\s+?--disconnected\s+?--skip-db-diskspace-check(\s+)?}m)
      end
    end
    ...
    describe 'with setup options' do
      ...
      it do
        is_expected.to contain_file('/usr/bin/spacewalk.sh').with(content: %r{^spacewalk-setup.+--answer-file=\/etc\/sysconfig\/spacewalk\.answer\s+?--non-interactive\s+?--disconnected\s+?--re-register\s+?--clear-db(\s+)?}m)
      end
    end
    ...
    describe 'with skip options' do
      ...
      it do
        is_expected.to contain_file('/usr/bin/spacewalk.sh').with(content: %r{^spacewalk-setup.+--answer-file=\/etc\/sysconfig\/spacewalk\.answer\s+?--non-interactive\s+?--disconnected\s+?--skip-db-diskspace-check\s+?--skip-selinux-test\s+?--skip-db-install\s+?--skip-db-population\s+?--skip-fqdn-test(\s+)?}m)
      end
    end
  end
end
